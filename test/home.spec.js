...
describe("homePage", () => {
  it("uses the first exact allowed host in examples", () => {
    const html = homePage({ mode: "worker", origin: "https://ogshot.example.com", allowedHosts: "*.example.com,example.com" });
    expect(html).toContain("https://ogshot.example.com/render.png?url=https%3A%2F%2Fexample.com%2Fposts%2F1");
  ...
  it("turns a wildcard into a www host when there is no exact host", () => {
    const html = homePage({ mode: "worker", origin: "https://ogshot.example.com", allowedHosts: "*.example.com" });
    expect(html).toContain("https%3A%2F%2Fwww.example.com%2Fposts%2F1");
  });

  it("warns when no hosts are configured", () => {
    expect(homePage({ mode: "worker", origin: "https://ogshot.example.com" })).toContain("none configured");
  });
