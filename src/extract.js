...
/**
 * Content-addressed cache key. Only the parts of the page that affect the image are
 * hashed, so per-request noise like CSRF tokens doesn't trigger re-renders.
 *
 * @param {URL} url
 * @param {string} html
 * @returns {Promise<string>}
 */
export async function contentKey(url, html) {
  const content = extractTemplate(html) ?? extractFallbackInputs(html);
  const data = new TextEncoder().encode(`${url.origin}${url.pathname}${url.search}\n${content}`);
